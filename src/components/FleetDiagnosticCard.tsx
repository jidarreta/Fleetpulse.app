...
import { AlertTriangle, Activity, Wrench, CheckCircle, ShieldAlert, Zap, Radio, RefreshCw } from 'lucide-react';
...
export default function FleetDiagnosticCard({ vehicleId = "c39a2b8e-4f10-4123-b1d3-9f88102a0142" }: { vehicleId?: string }) {
  const [activeVehicleId, setActiveVehicleId] = useState<string>(vehicleId);
  ...
  useEffect(() => {
    ...
    const wsUrl = `${protocol}//${host}/ws/telemetry/${activeVehicleId}`;
  ...
  if (loading || !riskData) {
    return (
      <div className="p-8 text-center text-slate-400 animate-pulse font-mono bg-slate-900 border border-slate-800 rounded-xl">
          ...
          <div className="flex items-center space-x-2 mt-0.5">
            ...
            <div className="flex items-center space-x-1">
              {['c39a2b8e-4f10-4123-b1d3-9f88102a0142', 'FP-042', 'FP-104'].map((id) => (
                ...
                >
                  {id === 'c39a2b8e-4f10-4123-b1d3-9f88102a0142' ? 'Default UUID' : id}
                </button>
              ))}
            </div>
          </div>
